			...
			<main class="col-lg-10 col-md-7 ms-md-auto px-4">
				...
				<section id="commandList">
					<div class="container">
						<% if (article != null){ %>
						<a
							href="boardReplyForm.boa?board_num=<%=article.getBOARD_NUM() %>&page=<%=nowPage %>" class="btn-link">[답변]
						</a> <a
							href="boardModifyForm.boa?board_num=<%=article.getBOARD_NUM() %>" class="btn-link">[수정]</a>
						<a
							href="boardDeleteForm.boa?board_num=<%=article.getBOARD_NUM() %>&page=<%=nowPage %>" class="btn-link">[삭제]</a>
						<a href="boardList.boa?page=<%=nowPage %>" class="btn-link">[목록]</a>
						<%}else{ %>
						게시물이 없습니다.
					</div>
					<%} %>
